...
void eval(char *cmdline)
{
	...
	// block SIGCHLD to handle immediate exit of child process
	// sigemptyset, sigaddset never fails
	sigemptyset(&sigset);
	sigaddset(&sigset, SIGCHLD);
	if(sigprocmask(SIG_BLOCK, &sigset, NULL) < 0)
	{
		// handle sigprocmask error
		perror("sigprocmask");
		// free allocated memory
		free(argv);
		return;
	...
	{
		// I'm a child
		// set new gid to distinguish from foreground process group
		if(setpgid(0, 0) < 0)
		{
			// handle setpgid error
			perror("setpgid");
		}
		...
		// procedure never comes here
		// if anything wrong so procedure falls here, it should exit immediately
		exit(0);
	}
	...
	// add job to job list before signaled
	addjob(jobs, pid, bg ? BG : FG, cmdline);

	// print job if background
	if(bg)
	{
	...
	// wait for current job if foreground
	if(!bg)
	{
		waitfg(pid);
	}
...
void do_bgfg(char **argv) 
{
	...
	pid_t pid;
	struct job_t *job;
	char *ptr;			// char pointer for strtol
	...
	if(argv[1])
	{
		if(argv[1][0] == '%')
		{
			...
			jid = strtol(argv[1] + 1, &ptr, 10);
			// check strtol
			if(ptr != argv[1] + strlen(argv[1]))
			{
				// argv[1] is not a proper integer
				printf("%s: argument must be a PID or %%jobid\n", argv[0]);
				return;
			}
			// find job object with jid
			job = getjobjid(jobs, jid);
			if(job == NULL)
			{
				// argv[1] is not a proper jid
		...
		else
		{
			...
			// convert argv[1] to an integer
			// ignore overflow or underflow
			pid = (pid_t)strtol(argv[1], &ptr, 10);
			// check strtol
			if(ptr != argv[1] + strlen(argv[1]))
			{
				// argv[1] is not a proper integer
...
void waitfg(pid_t pid)
{
	...
	// job might be NULL in case of immediate exit of child process
	if(job)
	{
		// pause procedure while foreground job is runnings
		// pause never fails
		for(; job->pid == pid && job->state == FG; pause());
	}
}
